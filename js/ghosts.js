...
LOGOS.bitcoin.src = 'img/bitcoin.png';
LOGOS.ethereum.src = 'img/ethereum.png';
LOGOS.tether.src = 'img/tether.png';
LOGOS.tron.src = 'img/tron.png';
LOGOS.afraid.src = 'img/afraid.png';  // Add this line to load the afraid image
...
function drawGhost(ghost) { 
    ...
    // Handle the afraid state first
    if (eval('GHOST_' + ghost.toUpperCase() + '_STATE !== 0')) { 
        if (eval('GHOST_' + ghost.toUpperCase() + '_AFFRAID_STATE === 1')) { 
            ctx.fillStyle = GHOST_AFFRAID_FINISH_COLOR; // Ghost is white when almost finished being afraid
        } else { 
            ctx.fillStyle = GHOST_AFFRAID_COLOR; // Ghost is blue when afraid
        }
    } else {
        // Set the color of each ghost based on the cryptocurrency
        if (ghost === 'blinky') { // Bitcoin
            ctx.fillStyle = '#F7931A'; // Bitcoin Orange
        } else if (ghost === 'pinky') { // Ethereum
            ctx.fillStyle = '#3C3C3D'; // Ethereum Blue
        } else if (ghost === 'inky') { // Tether
            ctx.fillStyle = '#26A17B'; // Tether Green
        } else if (ghost === 'clyde') { // Tron
            ctx.fillStyle = '#F50000'; // Tron Red
        }
    }

    // Draw the ghost's body using the same helper function (fills it with the selected color)
    ctx.beginPath();
    ...
    // Only draw the logos if the ghost is not afraid
    if (eval('GHOST_' + ghost.toUpperCase() + '_STATE === 0')) {
        // Increase the size of the logos
        var logoSize = 40; // Adjusted size to make the logos bigger

        // Draw the respective logos as images
        if (ghost === 'blinky') {
            ctx.drawImage(LOGOS.bitcoin, ghostX - logoSize / 2, ghostY - logoSize / 2, logoSize, logoSize);
        } else if (ghost === 'pinky') {
            ctx.drawImage(LOGOS.ethereum, ghostX - logoSize / 2, ghostY - logoSize / 2, logoSize, logoSize);
        } else if (ghost === 'inky') {
            ctx.drawImage(LOGOS.tether, ghostX - logoSize / 2, ghostY - logoSize / 2, logoSize, logoSize);
        } else if (ghost === 'clyde') {
            // Reduce the Tron logo size slightly
            var tronLogoSize = 35; // Smaller than the others
            ctx.drawImage(LOGOS.tron, ghostX - tronLogoSize / 2, ghostY - tronLogoSize / 2, tronLogoSize, tronLogoSize);
        }
    }

    // --- Replace the eyes with your provided code ---

    var eyesX = 0;
    var eyesY = 0;

    if (eval('GHOST_' + ghost.toUpperCase() + '_DIRECTION') === 4) { 
        eyesY = -5;
    } else if (eval('GHOST_' + ghost.toUpperCase() + '_DIRECTION') === 1) { 
        eyesX = +2;
    ...
    if (eval('GHOST_' + ghost.toUpperCase() + '_STATE === 0') || eval('GHOST_' + ghost.toUpperCase() + '_STATE === -1')) { 
        ...
        // Left eye
        ctx.beginPath();
        ctx.moveTo((ghostX - 15) + 8 + eyesX, (ghostY + 16) - 24 + eyesY);
        ctx.bezierCurveTo((ghostX - 15) + 5 + eyesX, (ghostY + 16) - 24 + eyesY, (ghostX - 15) + 4 + eyesX, (ghostY + 16) - 21 + eyesY, (ghostX - 15) + 4 + eyesX, (ghostY + 16) - 19 + eyesY);
        ctx.bezierCurveTo((ghostX - 15) + 4 + eyesX, (ghostY + 16) - 17 + eyesY, (ghostX - 15) + 5 + eyesX, (ghostY + 16) - 14 + eyesY, (ghostX - 15) + 8 + eyesX, (ghostY + 16) - 14 + eyesY);
        ctx.bezierCurveTo((ghostX - 15) + 11 + eyesX, (ghostY + 16) - 14 + eyesY, (ghostX - 15) + 12 + eyesX, (ghostY + 16) - 17 + eyesY, (ghostX - 15) + 12 + eyesX, (ghostY + 16) - 19 + eyesY);
        ctx.bezierCurveTo((ghostX - 15) + 12 + eyesX, (ghostY + 16) - 21 + eyesY, (ghostX - 15) + 11 + eyesX, (ghostY + 16) - 24 + eyesY, (ghostX - 15) + 8 + eyesX, (ghostY + 16) - 24 + eyesY);
        ctx.fill();
        ...
        // Right eye
        ctx.beginPath();
        ctx.moveTo((ghostX - 15) + 20 + eyesX, (ghostY + 16) - 24 + eyesY);
        ctx.bezierCurveTo((ghostX - 15) + 17 + eyesX, (ghostY + 16) - 24 + eyesY, (ghostX - 15) + 16 + eyesX, (ghostY + 16) - 21 + eyesY, (ghostX - 15) + 16 + eyesX, (ghostY + 16) - 19 + eyesY);
        ctx.bezierCurveTo((ghostX - 15) + 16 + eyesX, (ghostY + 16) - 17 + eyesY, (ghostX - 15) + 17 + eyesX, (ghostY + 16) - 14 + eyesY, (ghostX - 15) + 20 + eyesX, (ghostY + 16) - 14 + eyesY);
        ctx.bezierCurveTo((ghostX - 15) + 23 + eyesX, (ghostY + 16) - 14 + eyesY, (ghostX - 15) + 24 + eyesX, (ghostY + 16) - 17 + eyesY, (ghostX - 15) + 24 + eyesX, (ghostY + 16) - 19 + eyesY);
        ctx.bezierCurveTo((ghostX - 15) + 24 + eyesX, (ghostY + 16) - 21 + eyesY, (ghostX - 15) + 23 + eyesX, (ghostY + 16) - 24 + eyesY, (ghostX - 15) + 20 + eyesX, (ghostY + 16) - 24 + eyesY);
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc((ghostX - 15) + 18 + eyesX, (ghostY + 16) - 18 + eyesY, 2, 0, Math.PI * 2, true); // Left pupil
        ctx.fill();

        ctx.beginPath();
        ctx.arc((ghostX - 15) + 6 + eyesX, (ghostY + 16) - 18 + eyesY, 2, 0, Math.PI * 2, true); // Right pupil
        ctx.fill();
    }
}
