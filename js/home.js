...
function initHome() { 
	...
	// Drawing Pacman on the home screen
	var ctx = null;
	...
	var x = 50;
	var y = 50;
	ctx.fillStyle = "#fff200";  // Pacman color
	ctx.beginPath();
	...
	// Drawing Blinky (Bitcoin ghost)
	drawGhostWithLogo('canvas-presentation-blinky', "#F7931A", 'img/bitcoin.png'); // Bitcoin Orange and logo

	// Drawing Pinky (Ethereum ghost)
	drawGhostWithLogo('canvas-presentation-pinky', "#3C3C3D", 'img/ethereum.png'); // Ethereum Blue and logo

	// Drawing Inky (Tether ghost)
	drawGhostWithLogo('canvas-presentation-inky', "#26A17B", 'img/tether.png'); // Tether Green and logo

	// Drawing Clyde (Tron ghost)
	drawGhostWithLogo('canvas-presentation-clyde', "#F50000", 'img/tron.png'); // Tron Red and logo

	startPresentation();
}

function drawGhostWithLogo(canvasId, color, logoSrc) {
	var canvas = document.getElementById(canvasId);
	canvas.setAttribute('width', '50');
	canvas.setAttribute('height', '50');
	var ctx = canvas.getContext('2d');

	// Draw the ghost in the specified color
	ctx.fillStyle = color;
	drawHelperGhost(ctx, 25, 25, 1, 0, 0, 0);

	// Load the corresponding logo and draw it on the ghost
	var img = new Image();
	img.onload = function() {
		if (logoSrc === 'img/tron.png') {
			ctx.drawImage(img, 8, 10, 32, 32);  // Adjust the size of the Tron logo here (currently 35x35)
		} else {
			ctx.drawImage(img, 5, 5, 40, 40);  // Default size for other logos
		}

		// Draw the eyes on top of the logo
		drawGhostEyes(ctx, 25, 25, 1);  // Adjust the position and direction as needed
	};
	img.src = logoSrc;
}

// Draw ghost eyes with outlines (extracted from ghosts.js)
function drawGhostEyes(ctx, ghostX, ghostY, direction) {
    var eyesX = 0;
    var eyesY = 0;

    // Adjust eye position based on the ghost's direction (currently facing right)
    if (direction === 4) { 
        eyesY = -5;  // Moving upward
    } else if (direction === 1) { 
        eyesX = +2;  // Moving right
    } else if (direction === 2) { 
        eyesY = +5;  // Moving downward
    } else if (direction === 3) { 
        eyesX = -3;  // Moving left
    }
    ...
    // Pupils (centered in the eyes)
    ctx.fillStyle = "#0000fa";  // Blue pupils
    ctx.beginPath();
    ctx.arc((ghostX - 15) + 18 + eyesX, (ghostY + 16) - 18 + eyesY, 2, 0, Math.PI * 2, true); // Right pupil
    ctx.fill();

    ctx.beginPath();
    ctx.arc((ghostX - 15) + 6 + eyesX, (ghostY + 16) - 18 + eyesY, 2, 0, Math.PI * 2, true); // Left pupil
    ctx.fill();
}
